...
public class ProductServiceImpl implements ProductService {
	...
	public ProductDTO addProduct(Long categoryId, Long brandId, Product product) {
		...
		if (isProductNotPresent) {
			...
			boolean isProductNotPresentInBrand = true;

			List<Product> productsInBrand = brand.getProducts();

			for (int i = 0; i < productsInBrand.size(); i++) {
				if (products.get(i).getProductName().equals(product.getProductName())
						&& products.get(i).getDescription().equals(product.getDescription())) {
	
					isProductNotPresentInBrand = false;
					break;
				}
			}

			if (isProductNotPresentInBrand && isProductNotPresent) {
